...
The form ((1 . 3) (6 . 2)) means that bits 1 through 3 and bits 
6 through 7 are 1. A one index base is used for numbering.
The BOA-constructor for this structure is named, make-compressed. It 
constructs compressed bit pairs from one of four types: :list, :number
:comp, and :rep. Here, the argument to :list is a list of bits; to :number
is a number; to :comp is a list of run length encoded pairs representing
bits which are 1.; and the argument to :rep is a queue which represents 
a compressed bit string in normal form.

Example: (get-compressed-pairs (make-compressed :list '(0 1 0 0 1 1 1 0 0 1)))
((2 . 1) (5 . 3) (10 . 1))
Example: (get-compressed-pairs (make-compressed :number 11))
((1 . 2) (4 . 1))
The bits of the number are listed starting with the low bits first.
Example: (get-compressed-pairs (make-compressed :comp '((1 . 2) (2 . 3))))
((1 . 4))
Example: (get-compressed-pairs 
...
(defun diff (comps)
 ...
 Assumption: <comps> is a list of association pairs ordered 
  ...
  (labels ((diff-helper (start duration comps queue)
    ...
    (when comps
      (merge 
       (diff-helper (caar comps) (cdar comps) 
                    (cdr comps) (rsm.queue:create))))))
