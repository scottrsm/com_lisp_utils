...
;;;;
;;;; $Id: genetic-alg.lisp,v 1.5 2003/09/14 16:54:27 rscottmcintire Exp $
;;;; *************************************************************************
  ...
  (let* ((n0 0)
    ...
    (let ((idx
           (loop 
             ...
             (when (= (1+ n0) n1)
               (return
                 (if (<= val (funcall val-acc (svref vec-obj n0)))
                     n0
                   n1)))
             (setf mid (truncate (+ n0 n1) 2))
             (if (<= val (funcall val-acc (svref vec-obj mid)))
                 (setf n1 mid)
               (setf n0 mid)))))
      (funcall obj-acc (svref vec-obj idx)))))
...
two pieces."
  (let ((g1 (make-array point))
        (g2 (make-array (- (gene-length gene) point))))
    (loop for i from 0 below (gene-length g1) 
        as g-val across gene do
          (setf (svref g1 i) g-val))
    (loop for i from 0 below (gene-length g2) 
        as g-val across gene do
          (setf (svref g2 i) g-val))
    (values g1 g2)))
                  ...
                  (case op
                    ...
                    (0 
                     (loop for i from 0 below mutate-point do
                           (setf (svref new-gene i) (svref gene i)))
                     (loop for i from (1+ mutate-point) below len 
                         as j from 0 do
                           (setf (svref new-gene (+ j mutate-point))
                             (svref gene i))))
                    ...
                    (1 
                     (loop for i from 0 upto mutate-point do
                           (setf (svref new-gene i) (svref gene i)))
                     (setf (svref new-gene (1+ mutate-point))
                       (pick-random-alpha gene-alphabet))
                     (loop for i from (1+ mutate-point) below len 
                         as j from 2 do
                           (setf (svref new-gene (+ j mutate-point))
...
(defun make-gene-prob-dist (gene-list fitness-function &key (cache t))
  "Given a list of genes and a fitness function, return what effectively 
amounts to a gene cumulative probability distribution (not a density).  
That is, a vector of pairs: (gene . fitness) along with the total fitness 
of this vector of genes. If the key word <cache> has a non nil value, then
cache genes that are highly fit."
  (let ((cum 0)
