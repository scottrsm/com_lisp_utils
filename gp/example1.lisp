...
(defpackage gen-ex1
  (:use #:cl)
  (:documentation
   "Example of genetic programming used to do symbolic regression."))
...
(defparameter *gen-params* (rsm.gen-prog:make-gen-params 
                            *population-size* 
...
(let ((programs (rsm.gen-prog:get-best-programs *gen-params*)))
  ...
  (let* ((first (car programs))
    ...
    ;;; with the first best one we found.
    (format t "COMPARE the first BEST PROGRAM TREE with the REAL FUNCTION~%~%")
    (loop :for point :in *points* :do
      (let ((correct (apply #'real-func point))
            (best (apply best-one point)))
        (when (eql correct best)
          (incf correct-sum))
        (format t "first best = ~a~creal = ~a~%" 
                best
                #\Tab
                correct
                )))
