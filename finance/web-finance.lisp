...
;;;;
;;;; $Id: web-finance.lisp,v 1.2 2004/02/17 14:56:31 rscottmcintire Exp $
;;;; *************************************************************************
...
(defparameter *host* '("localhost" "24.60.102.246"))
  ...
  (let ((debt (read-from-string initial-debt nil nil))
    ...
    (let ((payment 
      ...
      (html
       ...
       ((:em :style "color=green") (:princ-safe (format nil "$~2$" payment)))))
      (values)))
    ...
    (html
     ...
     (let ((debt (format nil "~$~%" 
                          (calc-initial-debt 
                           ...
                           (read-from-string monthly-payment)))))
       (html
        ((:em :style "color=green") (:princ-safe (format nil "$~$" debt))))
       (set-loan-params (list (list "years" years-to-pay)
                              (list "rate" annual-interest-rate)
                              (list "payment" monthly-payment)
    ...
    (handler-case
        (let ((rate 
          ...
          (html
           ...
           ((:em :style "color=green") (:princ-safe (format nil "~3$" rate)))))
      (non-solveable (obj) 
        (html
         (:br)
         ((:b :style "color=red")
